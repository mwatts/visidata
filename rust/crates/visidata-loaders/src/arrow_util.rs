...
///
/// Integers → `Value::Int`, Floats/Decimals → `Value::Float`, Booleans → `Value::Bool`,
/// Dates/Timestamps → `Value::Date`, Strings → `Value::Text`, Binary → `Value::Bytes`,
/// Dictionary → recurse, Null / nested → `Value::Null` / `Value::Text`.
///
/// # Panics
///
...
pub fn arrow_value_at(array: &Arc<dyn Array>, idx: usize) -> Value {
    ...
    match array.data_type() {
        ...
        ArrowType::Boolean => Value::Bool(array.as_any().downcast_ref::<BooleanArray>().unwrap().value(idx)),
        ArrowType::Int8  => Value::Int(i64::from(array.as_any().downcast_ref::<Int8Array>().unwrap().value(idx))),
        ArrowType::Int16 => Value::Int(i64::from(array.as_any().downcast_ref::<Int16Array>().unwrap().value(idx))),
        ArrowType::Int32 => Value::Int(i64::from(array.as_any().downcast_ref::<Int32Array>().unwrap().value(idx))),
        ArrowType::Int64 => Value::Int(array.as_any().downcast_ref::<Int64Array>().unwrap().value(idx)),
        ArrowType::UInt8  => Value::Int(i64::from(array.as_any().downcast_ref::<UInt8Array>().unwrap().value(idx))),
        ArrowType::UInt16 => Value::Int(i64::from(array.as_any().downcast_ref::<UInt16Array>().unwrap().value(idx))),
        ArrowType::UInt32 => Value::Int(i64::from(array.as_any().downcast_ref::<UInt32Array>().unwrap().value(idx))),
        ArrowType::UInt64 => {
            let v = array.as_any().downcast_ref::<UInt64Array>().unwrap().value(idx);
            i64::try_from(v).map_or_else(|_| Value::Text(v.to_string()), Value::Int)
        }
        ArrowType::Float16 => Value::Float(f64::from(array.as_any().downcast_ref::<Float16Array>().unwrap().value(idx))),
        ArrowType::Float32 => Value::Float(f64::from(array.as_any().downcast_ref::<Float32Array>().unwrap().value(idx))),
        ArrowType::Float64 => Value::Float(array.as_any().downcast_ref::<Float64Array>().unwrap().value(idx)),
        ArrowType::Decimal128(precision, scale) => {
            decimal128_to_float(array.as_any().downcast_ref::<Decimal128Array>().unwrap().value(idx), *precision, *scale)
        }
        ArrowType::Decimal256(precision, scale) => {
            let s = format_decimal256(array.as_any().downcast_ref::<Decimal256Array>().unwrap().value(idx), *precision, *scale);
            s.parse::<f64>().map(Value::Float).unwrap_or(Value::Text(s))
        }
        ArrowType::Date32 => date32_to_value(array.as_any().downcast_ref::<Date32Array>().unwrap().value(idx)),
        ArrowType::Date64 => date64_to_value(array.as_any().downcast_ref::<Date64Array>().unwrap().value(idx)),
        ArrowType::Timestamp(unit, _tz) => timestamp_to_value(array, idx, *unit),
        #[expect(clippy::match_same_arms, reason = "clarity: each arm is a distinct type category")]
        ArrowType::Time32(_) | ArrowType::Time64(_) | ArrowType::Duration(_) | ArrowType::Interval(_) => display_value(array, idx),
        ArrowType::Utf8     => Value::Text(array.as_any().downcast_ref::<StringArray>().unwrap().value(idx).to_owned()),
        ArrowType::LargeUtf8 => Value::Text(array.as_any().downcast_ref::<LargeStringArray>().unwrap().value(idx).to_owned()),
        ArrowType::Utf8View => Value::Text(array.as_string_view().value(idx).to_owned()),
        ArrowType::Binary     => Value::Bytes(array.as_any().downcast_ref::<BinaryArray>().unwrap().value(idx).to_vec()),
        ArrowType::LargeBinary => Value::Bytes(array.as_any().downcast_ref::<LargeBinaryArray>().unwrap().value(idx).to_vec()),
        ArrowType::BinaryView => Value::Bytes(array.as_any().downcast_ref::<BinaryViewArray>().unwrap().value(idx).to_vec()),
        ArrowType::FixedSizeBinary(_) => Value::Bytes(array.as_any().downcast_ref::<FixedSizeBinaryArray>().unwrap().value(idx).to_vec()),
        ArrowType::Dictionary(_, _) => arrow_dict_value(array, idx),
        _ => display_value(array, idx),
    }
}

/// Decode a dictionary-encoded cell by casting to its value type and recursing.
fn arrow_dict_value(array: &Arc<dyn Array>, idx: usize) -> Value {
    let value_type = match array.data_type() {
        ArrowType::Dictionary(_, v) => v.as_ref().clone(),
        _ => return display_value(array, idx),
    };
